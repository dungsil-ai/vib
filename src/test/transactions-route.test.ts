...
describe('GET /api/transactions', () => {
  ...
  it('year/month 레거시 파라미터는 UTC 월 범위로 배열을 직접 반환한다', async () => {
    ...
    const findManyCalls = vi.mocked(prisma.transaction.findMany).mock.calls
    const where = findManyCalls[0][0]?.where as Record<string, { gte?: Date; lte?: Date }>
    expect(where.date?.gte?.toISOString()).toBe('2024-01-01T00:00:00.000Z')
    expect(where.date?.lte?.toISOString()).toBe('2024-01-31T23:59:59.999Z')
  })
...
describe('POST /api/transactions', () => {
  ...
  it('외화 항목에 선행 0 없는 소수 환율(.5) 입력 시 201을 반환한다', async () => {
    ...
    const req = new NextRequest('http://localhost/api/transactions', {
      ...
      body: JSON.stringify({
        date: '2024-01-15',
        description: '테스트',
        entries: [{ debitAccountId: 'acc-1', creditAccountId: 'acc-2', amount: '100', currency: 'USD', exchangeRate: '.5' }],
      }),
    })
    const res = await POST(req)
    expect(res.status).toBe(201)
  ...
  it('외화 항목에 후행 점 환율(1.) 입력 시 201을 반환한다', async () => {
    const createdTx = {
      id: 'tx-1',
      date: new Date('2024-01-15'),
      description: '테스트',
    ...
    const req = new NextRequest('http://localhost/api/transactions', {
      ...
      body: JSON.stringify({
        date: '2024-01-15',
        description: '테스트',
        entries: [{ debitAccountId: 'acc-1', creditAccountId: 'acc-2', amount: '100', currency: 'USD', exchangeRate: '1.' }],
      }),
    })
    const res = await POST(req)
    expect(res.status).toBe(201)
  ...
  it('외화 항목에 숫자 타입 환율 입력 시 201을 반환한다', async () => {
    const createdTx = {
      id: 'tx-1',
      date: new Date('2024-01-15'),
      description: '테스트',
    ...
    const req = new NextRequest('http://localhost/api/transactions', {
      ...
      body: JSON.stringify({
        date: '2024-01-15',
        description: '테스트',
        entries: [{ debitAccountId: 'acc-1', creditAccountId: 'acc-2', amount: '100', currency: 'USD', exchangeRate: 1300.25 }],
      }),
    })
    const res = await POST(req)
    expect(res.status).toBe(201)
