...
export default function RecurringTransactionsPage() {
  ...
  const fetchRecurring = async (isCancelled: () => boolean = () => false) => {
    try {
      if (!isCancelled()) setListError(null)
      const res = await fetch('/api/recurring-transactions')
      if (!res.ok) throw new Error(`반복 거래를 불러오지 못했습니다. (${res.status})`)
      const data = await res.json()
      if (!isCancelled()) setRecurringList(data)
    } catch (err) {
      if (!isCancelled()) setListError(err instanceof Error ? err.message : '반복 거래를 불러오는 중 오류가 발생했습니다.')
    } finally {
      if (!isCancelled()) setListLoading(false)
    }
  }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (!cancelled) setAccountsError(err instanceof Error ? err.message : '계정 목록을 불러오는 중 오류가 발생했습니다.')
      } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">반복 거래</h1>
        <button
